...
const handler = async (m, {conn, command}) => {
  if (!db.data.chats[m.chat].modohorny && m.isGroup) throw '*NSFW desactivado para este chat.*';

  switch (command) {
    case 'pack':
      const url = pack[Math.floor(Math.random() * pack.length)];
      await conn.sendMessage(m.chat, {image: {url}, caption: '_🥵 Pack 🥵_'}, {quoted: m});
      break;
    case 'pack2':
      const url2 = packgirl[Math.floor(Math.random() * packgirl.length)];
      await conn.sendMessage(m.chat, {image: {url: url2}, caption: '_🥵 Pack 🥵_'}, {quoted: m});
      break;
    case 'pack3':
      const url3 = packmen[Math.floor(Math.random() * packmen.length)];
      await conn.sendMessage(m.chat, {image: {url: url3}, caption: '_🥵 Pack 3 🥵_'}, {quoted: m});
      break;
    case 'videoxxx': case 'vídeoxxx':
      const url4 = videosxxxc[Math.floor(Math.random() * videosxxxc.length)];
      await conn.sendMessage(m.chat, {video: {url: url4}, caption: '*Video NSFW* 🥵'}, {quoted: m});
      break;
    case 'videoxxxlesbi': case 'videolesbixxx': case 'pornolesbivid': case 'pornolesbianavid': case 'pornolesbiv': case 'pornolesbianav': case 'pornolesv':
      ...
      await conn.sendMessage(m.chat, {video: {url: url5}, caption: '*Video NSFW* 🥵'}, {quoted: m});
      break;
  }
};
